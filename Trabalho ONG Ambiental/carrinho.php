...
$usuario_id_para_carrinho = null;
if (isset($_SESSION['usuario_logado_id'])) {
    $usuario_id_para_carrinho = $_SESSION['usuario_logado_id'];
} elseif (isset($_SESSION['logado'])) {
    $usuario_id_para_carrinho = 1; 
}


if (isset($usuario_id_para_carrinho)) {
    ...
    $stmt_carrinho_id->bind_param("i", $usuario_id_para_carrinho);
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['remove_item_id'])) {
    ...
    if ($item_id_to_remove !== false && isset($usuario_id_para_carrinho)) {
        ...
        if ($stmt_check_owner) {
            $stmt_check_owner->bind_param("ii", $item_id_to_remove, $usuario_id_para_carrinho);
...
<html lang="pt-br">
    <body class="d-flex flex-column min-vh-100">
        <main class="flex-grow-1 container my-5 p-4 bg-white rounded shadow-sm">
            <h2 class="text-center mb-4" style="color: var(--verde-principal);">Seu Carrinho de Compras</h2> <?php if (!empty($mensagem_carrinho)): ?>
                <div class="alert alert-<?= $tipo_mensagem_carrinho ?> text-center" role="alert">
                    <?= $mensagem_carrinho ?>
                </div>
            <?php endif; ?>

            <?php if (!isset($_SESSION['usuario_logado_id']) && !isset($_SESSION['logado'])): ?> <div class="alert alert-info text-center" role="alert">
